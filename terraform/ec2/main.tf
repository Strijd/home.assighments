variable "env_name" {}
variable "vpc_public_security_group_ids" {type = list(string)}
variable "vpc_private_security_group_ids" {type = list(string)}
...
resource "aws_instance" "this_instance" {
  ...
  vpc_security_group_ids      = var.ec2_group[count.index]["type"] == "public" ? var.vpc_public_security_group_ids : var.vpc_private_security_group_ids
